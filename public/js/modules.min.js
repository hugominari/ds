...
$(document).ajaxSend(function (e, xhr, settings) {
	...
	var urlsNonBlock = ['users/verify-notify'];
...
try {
	var _loadMasks = function _loadMasks() {
		...
		var CpfCnpjMaskBehavior = function CpfCnpjMaskBehavior(val) {
		    ...
		    cpfCnpjpOptions = {
			onKeyPress: function onKeyPress(val, e, field, cpfCnpjpOptions) {
...
function generateModal(title, text, type, callback) {
	...
	swal(config).then(function (result) {
		if (result.value && !!callback) callback(true);else callback(false);
	});
}
/**
 ...
 */
try {
	var _loadDropzone = function _loadDropzone(element) {
		if (!element) element = $("[data-type='dropzone']");
		...
		element.each(function (e) {
			...
			dz.dropzone({
				...
				init: function init() {
					...
					this.on('addedfile', function (file) {
						if (!empty(blockEl)) blockUI(blockEl);
					});
					...
					this.on('error', function (file, data, xhr) {
						if (!(this.files.length > maxFiles)) dzObj.removeAllFiles();

						var maxFileSize = this.options.maxFilesize * 1024 * 1024;
						var maxMB = bytesToSize(maxFileSize);
						var fileMB = bytesToSize(file.size);

						if (file.size > maxFileSize) generateNotify('Erro: ', 'O arquivo excedeu o tamanho máximo permitido: (' + maxMB + ') e o tamanho dele é de ' + fileMB, 'error');

						//Display message
						if (!!data.message) {
							dz.find('.dz-message').children(0).hide().after('<span>' + data.message + '</span>');
						}
					});

					this.on('success', function (file, data) {
						//Disable dropzone
						if (data.success) dzObj.disable();

						//Set callback if exist
						if (!!data.callback) Callbacks[data.callback](data);
					...
					this.on('complete', function (file, data) {
						if (!empty(blockEl)) unblockUI(blockEl);

						dz.find('.dz-preview .dz-remove').removeClass('dz-sending');

						if (dz.hasClass('with-error')) dz.removeClass('with-error');
						...
						if (path.length > 4 && !keepFiles && !str_contains(path, '/0/')) {
							...
							$.ajax({
								...
								success: function success() {
									...
									if (dz.hasClass('dz-multiple') && qtd > 1) dz.parent().remove();
								}
							});
						}
			...
			if (dz.attr('disabled') || dz.hasClass('disabled')) dzObj.disable();
		});
	};
...
try {
	...
	var modalTemplate = '' + '<div id="modal-cropper" class="modal fade" tabindex="-1" role="dialog">' + '<div class="modal-dialog" role="document">' + '<div class="modal-content">' + '<div class="modal-header">' + '<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>' + '<h4 class="modal-title">Recorte de imagem</h4>' + '</div>' + '<div class="modal-body">' + '<div class="image-container"></div>' + '</div>' + '<div class="modal-footer">' + '<button type="button" class="btn btn-default" data-dismiss="modal">Fechar</button>' + '<button type="button" class="btn btn-primary m-r-18 crop-upload">Salvar</button>' + '</div>' + '</div>' + '</div>' + '</div>' + '';
	...
	$("[data-type='dropzone-crop']").each(function (e) {
		...
		dz.dropzone({
			...
			init: function init() {
				...
				if (dz.hasClass('disabled')) dz.unbind('click');
				...
				this.on('removedfile', function (file, data) {
					...
					if (path.length > 4 && !keepFiles && !str_contains(path, '/0/')) {
						...
						$.ajax({
							...
							data: data,
							success: function success() {
								if (dz.hasClass('dz-multiple')) dz.parent().remove();
							}
						});
					}
...
var wrapper = $('.js-filter-box');
var lineFilter = wrapper.children('.row.line').clone();
var form = $('#form-filter');

//Constants
var FILTER_EQUAL = '1';
var FILTER_CONTAINS = '2';
var FILTER_NOT_EQUAL = '3';
var FILTER_START_WITH = '4';
var FILTER_END_WITH = '5';
var FILTER_GREATER_THAN = '6';
var FILTER_GREATER_OR_EQUAL = '7';
var FILTER_LESS_THAN = '8';
var FILTER_LESS_OR_EQUAL = '9';
var FILTER_DATE_GREATER_THAN = '10';
var FILTER_DATE_LESS_THAN = '11';
...
$('.js-add-filter').on('click', function () {
	...
	newLine.find('.js-add-filter').addClass('js-remove-filter').removeClass('js-add-filter').find('.fa-plus').addClass('fa-minus').removeClass('fa-plus');
	...
	switch (type) {
		case 'number':
			operatorChilds.each(function () {
				var value = $(this).val();
				var correctValues = [FILTER_LESS_THAN, FILTER_GREATER_THAN, FILTER_LESS_OR_EQUAL, FILTER_GREATER_OR_EQUAL, FILTER_EQUAL];

				if (in_array(value, correctValues)) $(this).removeAttr('disabled');else $(this).attr('disabled', true);
			});
		...
		case 'select':
			var selectVals = $('<select/>').attr({ 'name': 'value[]', 'data-name': 'value' });
			var valuesText = JSON.parse(values);
			var objValues = [];

			//Disable operators
			operatorChilds.each(function () {
				var value = $(this).val();
				if (value !== FILTER_EQUAL) $(this).attr('disabled', true);else $(this).removeAttr('disabled').prop('selected', true);
			});
		...
		case 'datetime':
			operatorChilds.each(function () {
				var value = $(this).val();
				var correctValues = [FILTER_DATE_GREATER_THAN, FILTER_DATE_LESS_THAN, FILTER_EQUAL];
				if (in_array(value, correctValues)) $(this).removeAttr('disabled');else $(this).attr('disabled', true);
			});
		...
		default:
			operatorChilds.each(function () {
				var value = $(this).val();
				var correctValues = [FILTER_EQUAL, FILTER_CONTAINS, FILTER_NOT_EQUAL, FILTER_START_WITH, FILTER_END_WITH];
...
try {
	$("table[data-type='datatables']").each(function (e) {
		...
		//Default sort
		if (thSort.length > 0) defaultSort = parseInt(thSort.index());
	...
	if (Callbacks.preSubmit()) {
		...
		$.ajax({
			...
			success: function success(data) {
				...
				if (data.success) {
					if (!!data.callback) Callbacks[data.callback](data);else if (!!data.message) generateNotify(data.title, data.message, data.type);

					$('.clear-inputs').trigger('click');
					form.trigger('reset');

					if ($('[data-type="datatables"]').length > 0) $('[data-type=datatables]').DataTable().ajax.reload();
			...
			error: function error(jqXhr, json, errorThrown) {
				...
				if (!!response) {
					...
					$.each(response, function (index, value) {
						...
						if (isDropzone) boxError.find('.dropzone').addClass('with-error');else boxError.addClass('has-error');
						...
						if (type == 'checkbox' || type == 'radio') {
							...
							boxError.append('<span class="text-danger d-block span-error">' + '<i class="fa fa-exclamation-circle m-r-5" aria-hidden="true"></i>' + value + '</span>');
						} else {
							boxError.find('.span-error').remove();
							boxError.append('<span class="text-danger d-block span-error">' + '<i class="fa fa-exclamation-circle m-r-5" aria-hidden="true"></i>' + value + '</span>');
						}
...
$(document).on("click", ".js-delete", function (e) {
	...
	if (deleteUrl) {
		generateModal('Confirmar exclusão', 'Você deseja realmente excluir este registro?', 'info', function (callback) {
			if (callback) {
				var data = {
					'id': $('#id').val(),
					'type': $('#type').val(),
...
$(document).on("click", ".clear-inputs, [type=reset]", function (e) {
	...
	if (id && notClear) notClear.val(id);
});
