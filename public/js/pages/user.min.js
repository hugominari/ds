...
$(document).ready(function () {
	...
	if (dev.readOnly) {
		setTimeout(function () {
			var perms = $(".permissions-box input");
 ...
 */
function loadPermissions(roleId, cssSelector) {
						...
						if (_.size(permissions) > 0) {
							...
							for (var i = 0; i <= _.size(permissions); i++) {
								...
								if (typeof permission !== "undefined") {
									...
									if (permission.selected) permissionsArray.push(permission.id);
								}
							}

							if (dev.showOnly) baseHTML += "<input type='hidden' value='[" + permissionsArray + "]' name='permissions[]'>";
						} else {
