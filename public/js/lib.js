...
function blockUI(e) {
	...
	if (isButton || $this.hasClass('js-submit')) {
		$this.attr('disabled', true).attr('old-html', $this.html()).html('<i class="fa fa-spin fa-spinner"></i>' + ($this.hasClass('js-submit-icon-only') ? '' : ' Aguarde'));
	} else {

		$this.block({
			message: '<div class="loading-block">' + '<main>\n' + '    <heart>\n' + '        <span class=\'heartL\'></span>\n' + '        <span class=\'heartR\'></span>\n' + '        <span class=\'square\'></span>\n' + '    </heart>\n' + '    <shadow></shadow>\n' + '</main>' + '</div>',
			// theme: true,
			baseZ: 2000,
			css: {
				border: 'none',
...
function unblockUI(e) {
	...
	if (isButton || $this.hasClass('js-submit')) {
		$this.attr('disabled', false).html($this.attr('old-html'));
	} else {
		$this.unblock();
	}
}
...
window.Callbacks = {
  ...
  */
	redirect: function redirect(data) {
		if (!!data.time) {
			if (!!data.message && !!data.type) generateNotify(data.title, data.message, data.type);

			setTimeout(function () {
				if (!!data.url) window.location.href = data.url;else document.location.reload(true);
			}, data.time);
		} else {
			if (!!data.url) window.location.href = data.url;else document.location.reload(true);
		}
...
$(document).on('keyup', 'input.cep', function (ev) {
     ...
     */
				...
				if (data.success) {
					...
					if (typeof state.select2 == 'function') state.select2;
				}
			},
...
function setInputError(input, error) {
	...
	if (!input.hasClass('select2-focusser')) {
		if (isSelect2) target = input.prev();

		input.parents('.form-group').addClass('has-error tip');

		target.attr('title', error).tooltip({
			placement: 'bottom',
			trigger: 'hover focus'
		});
...
function removeInputError(input) {
	...
	if (input.hasClass('js-select')) input.prev().tooltip('destroy');
}
/**
...
function str_contains(str, value) {
	if (~str.indexOf(value)) return true;
